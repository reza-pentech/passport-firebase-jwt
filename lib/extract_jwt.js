...
var AUTH_HEADER = "authorization",
    ...
    CUSTOM_HEADER = 'x-identity-platform-client-id';
...
extractors.fromCustomAuthHeader = function () {
    return function (request) {

        var token = null;
        if (request.headers[CUSTOM_HEADER]) {
            return request.headers[CUSTOM_HEADER];
        }
        return token;
    };
};
...
extractors.fromAuthHeaderAsBearerToken = function () {
    return [extractors.fromAuthHeaderWithScheme(BEARER_AUTH_SCHEME), extractors.fromCustomAuthHeader()];
};
